...
void main() {
  ...
  test(
    'upsertUser saves user to offline SQLite and enqueues sync queue item',
    () async {
  ...
  test('update user modifies fields and enqueues sync UPDATE', () async {
    ...
    await repository.update(
      'user_1',
      UsersCompanion(name: const Value('Updated Name'), updatedAt: Value(now)),
    );
  ...
  test(
    'watchUser, watchById, and watchAll stream user profile updates',
    () async {
      const appUser = AppUser(
        uid: 'user_1',
  ...
  test(
    'delete user removes record from SQLite and enqueues sync DELETE',
    () async {
      const appUser = AppUser(
        uid: 'user_to_delete',
