...
class TestSupabaseQueryBuilder extends Fake implements SupabaseQueryBuilder {
  ...
  PostgrestFilterBuilder<dynamic> upsert(
    ...
    if (values is Map<String, dynamic>) {
      if (client.failOnRecordId != null &&
          values['id'] == client.failOnRecordId) {
        throw Exception(
          'Simulated failure for record ${client.failOnRecordId}',
        );
      }
  ...
  PostgrestFilterBuilder<dynamic> delete({String? count}) {
    ...
    return TestPostgrestFilterBuilder(
      onEq: (column, value) {
        if (column == 'id') {
          if (client.failOnRecordId != null &&
              value.toString() == client.failOnRecordId) {
            throw Exception(
              'Simulated delete failure for record ${client.failOnRecordId}',
            );
          }
...
class TestPostgrestFilterBuilder<T> extends Fake
...
void main() {
  ...
  group(
    'EMPIRICAL STRESS TEST 1: SyncQueue Enqueueing Across All Domain Repositories',
    () {
      test(
        'Offline repo mutations enqueue SyncQueueItems across all 10 domain repositories',
        () async {
          connectivityService.isConnected = false; // Device is offline
          final now = DateTime.now().toUtc().toIso8601String();

          // 1. Semesters Repository
          final semestersRepo = SemesterRepository(
            database,
            syncQueueRepository,
          );
          final semId = await semestersRepo.create(
            SemestersCompanion(
          ...
          final subId = await subjectsRepo.create(
            SubjectsCompanion(
          ...
          final attId = await attendanceRepo.create(
            AttendanceCompanion(
          ...
          final ttId = await timetableRepo.create(
            TimetableCompanion(
          ...
          final asgnId = await assignmentsRepo.create(
          ...
          final calId = await calendarRepo.create(
          ...
          await calendarRepo.update(
            ...
            const CalendarEventsCompanion(
              title: Value('Midterm Exam Room 302'),
            ),
          );

          // 7. Resources Repository
          final resourcesRepo = ResourcesRepository(
            database,
            syncQueueRepository,
          );
          final resId = await resourcesRepo.create(
            ResourcesCompanion(
          ...
          final internalMarksRepo = InternalMarksRepository(
            database,
            syncQueueRepository,
          );
          final markId = await internalMarksRepo.create(
          ...
          final updatedPending = await syncQueueRepository.getPendingItems(
            limit: 100,
          );
          final deleteItems = updatedPending
  ...
  group(
    'EMPIRICAL STRESS TEST 2: Exponential Backoff & Max Retry Limit Enforcement',
    () {
      test(
        'Empirically measures backoff formula 2^retryCount * 500ms and max retry limit (5 retries)',
        () async {
          ...
          expect(
            items.first.error,
            contains('Supabase connection network failure'),
          );
          ...
          await syncQueueRepository.recordFailure(
            id,
            'Fatal error attempt 5',
            4,
          ); // becomes retryCount 5
          ...
          expect(
            supabaseClient.upsertedPayloads['semesters'],
            isNull,
          ); // Was skipped!
        },
      );
  ...
  group('EMPIRICAL STRESS TEST 3: Connectivity Status Transition Auto-Flush', () {
    test(
      'Offline to Online auto-flushes pending queue, handles rapid toggle',
      () async {
  ...
  group('EMPIRICAL STRESS TEST 4: Local-First Truth & Crash Resilience', () {
    test(
      'Preserves local SQLite as source of truth and survives item failure mid-batch',
      () async {
        final now = DateTime.now().toUtc().toIso8601String();
        ...
        final localSubjects = await (database.select(
          database.subjects,
        )..where((t) => t.userId.equals('user_crash'))).get();
        expect(localSubjects.length, 3);
        expect(
          localSubjects.map((s) => s.name),
          contains('Local First Subject 1'),
        );
    ...
    test(
      'Gracefully handles orphan queue item when local row was hard deleted',
      () async {
        await syncQueueRepository.enqueue(
          targetTable: 'semesters',
