...
void main() {
  ...
  test('update semester updates name and enqueues sync UPDATE', () async {
    ...
    await repository.update(
      'user_1',
      'sem_1',
      SemestersCompanion(name: const Value('New Name'), updatedAt: Value(now)),
    );
  ...
  test(
    'setCurrent transaction unsets active semester and sets target semester as current',
    () async {
      final now = DateTime.now().toUtc().toIso8601String();
  ...
  test(
    'delete semester performs cascade soft-delete transaction on all child records',
    () async {
      final now = DateTime.now().toUtc().toIso8601String();
