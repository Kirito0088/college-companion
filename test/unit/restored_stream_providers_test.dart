...
void main() {
  group('SafeBunkCalculator Unit Tests', () {
    ...
    test('calculate calculates safe bunks when above target percentage (75%)', () {
    ...
    test('calculate calculates mustAttend when below target percentage (75%)', () {
  ...
  group('Restored StreamProviders Unit Tests', () {
    ...
    test('assignmentsStreamProvider emits list of assignments for user', () async {
      ...
      final assignments = await container.read(assignmentsStreamProvider(testUserId).future);
      expect(assignments.length, 1);
      expect(assignments.first.title, 'Math Homework');
      expect(assignments.first.userId, testUserId);
    });

    test('safeBunkStreamProvider calculates safe bunks from database attendance records', () async {
      final now = DateTime.now().toUtc().toIso8601String();
      ...
      final safeBunkResult = await container.read(safeBunkStreamProvider(testUserId).future);
      expect(safeBunkResult.attended, 8);
      expect(safeBunkResult.total, 10);
      expect(safeBunkResult.currentPercentage, 80.0);
    });

    test('calendarEventsStreamProvider emits list of events for user', () async {
      final now = DateTime.now().toUtc().toIso8601String();
      ...
      final events = await container.read(calendarEventsStreamProvider(testUserId).future);
    ...
    test('resourcesStreamProvider emits list of resources for user', () async {
      ...
      final resources = await container.read(resourcesStreamProvider(testUserId).future);
    ...
    test('userSettingsStreamProvider emits user settings for user', () async {
      ...
      final settings = await container.read(userSettingsStreamProvider(testUserId).future);
      expect(settings, isNotNull);
      expect(settings?.userId, testUserId);
    });

    test('dashboardSnapshotProvider produces fallback mock snapshot when userId is empty', () async {
    ...
    test('dashboardSnapshotProvider synthesizes real dashboard data for valid userId', () async {
      ...
      await db.into(db.calendarEvents).insert(
        CalendarEventsCompanion.insert(
          ...
          startDate: nowIso,
          endDate: now.add(const Duration(hours: 1)).toUtc().toIso8601String(),
          eventType: 'academic',
          createdAt: nowIso,
          updatedAt: nowIso,
        ),
