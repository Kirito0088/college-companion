...
final dashboardSnapshotProvider =
  ...
  final calendarEvents =
      await ref.watch(calendarEventsStreamProvider(userId).future);
  final assignments =
      await ref.watch(assignmentsStreamProvider(userId).future);
  ...
  for (final event in todayEvents) {
    ...
    final end =
        DateTime.tryParse(event.endDate) ?? start.add(const Duration(hours: 1));
  ...
  final upcomingList = sortedAssignments.take(3).map((a) {
    ...
    return DashboardAssignment(
      ...
      title: a.title,
      subject: subjectName,
      dueDateString: dueDate != null ? DateFormat('MMM d, yyyy').format(dueDate) : 'No due date',
  ...
  final workloadState =
      pendingAssignments.length > 3 ? 'Heavy' : 'Manageable';
