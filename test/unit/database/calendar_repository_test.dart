...
void main() {
  ...
  test(
    'create and getById calendar event with sync queue integration',
    () async {
  ...
  test(
    'update calendar event modifies title and enqueues sync UPDATE',
    () async {
      final now = DateTime.now().toUtc().toIso8601String();
  ...
  test(
    'soft delete calendar event enqueues sync DELETE and filters out event',
    () async {
      final now = DateTime.now().toUtc().toIso8601String();
  ...
  test(
    'watchByDateRange, watchUpcoming, and watchById filter calendar events correctly',
    () async {
      final now = DateTime.now().toUtc().toIso8601String();
      ...
      final rangeList = await repository
          .watchByDateRange(
            'user_1',
      ...
      final upcomingList = await repository
          .watchUpcoming('user_1', DateTime.utc(2026, 8, 15))
          .first;
