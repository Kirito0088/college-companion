...
void main() {
  ...
  group('Edge Case Tests - Malformed Payload & Input Handling', () {
    test(
      'Handles Unicode characters, emojis, and special symbols in fields',
      () async {
        final repo = AssignmentRepository(database, syncQueueRepository);
        final now = DateTime.now().toUtc().toIso8601String();

        const unicodeTitle =
            '📚 Math H.W. #1: 🔥 "Calculus & ∑(x^2)" <script>alert(1)</script>';
    ...
    test(
      'Handles unknown targetTable gracefully during sync queue processing',
      () async {
        await syncQueueRepository.enqueue(
  ...
  group('Edge Case Tests - Duplicate Record Conflicts', () {
    test(
      'UserRepository upsertUser handles duplicate user IDs cleanly without duplicate key crash',
      () async {
    ...
    test(
      'SubjectRepository throws DatabaseException when violating unique constraint (userId, semesterId, name)',
      () async {
    ...
    test(
      'UserSettingsRepository handles repeated saveSettings for same user ID',
      () async {
