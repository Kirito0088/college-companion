...
void main() {
  ...
  test(
    'create and getById timetable slot with sync queue integration',
    () async {
  ...
  test(
    'update timetable slot modifies fields and enqueues sync UPDATE',
    () async {
      final now = DateTime.now().toUtc().toIso8601String();
  ...
  test(
    'watchAll, watchByDay, watchBySubject, watchById streams filter and order correctly',
    () async {
      final now = DateTime.now().toUtc().toIso8601String();
      ...
      final mathList = await repository
          .watchBySubject('user_1', 'subj_math')
          .first;
