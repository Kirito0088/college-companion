...
void main() {
  ...
  group('Group 1: Reactive Emission Empirical Stress Tests', () {
    ...
    test(
      'assignmentsStreamProvider emits on insert, update, and soft-delete',
      () async {
        final emittedValues = <List<AssignmentEntity>>[];
        final subscription = container
            .listen<AsyncValue<List<AssignmentEntity>>>(
    ...
    test(
      'safeBunkStreamProvider emits updated calculations on attendance insert, update, and soft-delete',
      () async {
        ...
        await (db.update(
          db.attendance,
        )..where((t) => t.id.equals('att_p_0'))).write(
          AttendanceCompanion(
        ...
        await (db.update(db.attendance)..where((t) => t.id.equals('att_p_0')))
            .write(AttendanceCompanion(deletedAt: Value(nowIso)));
        await pumpEventQueue();
        ...
        final afterUpdate = emittedResults.firstWhere(
          (r) => r.total == 4 && r.attended == 3,
        );
    ...
    test(
      'calendarEventsStreamProvider emits on insert, update, and soft-delete',
      () async {
        final emittedValues = <List<CalendarEventEntity>>[];
        final subscription = container
            .listen<AsyncValue<List<CalendarEventEntity>>>(
        ...
        await repo.update(
          testUserId,
          id,
          const CalendarEventsCompanion(title: Value('Advanced Physics Lab')),
        );
        await pumpEventQueue();
    ...
    test(
      'resourcesStreamProvider emits on insert, update, and soft-delete',
      () async {
        ...
        await repo.update(
          testUserId,
          id,
          const ResourcesCompanion(title: Value('Updated Chapter 1 Notes')),
        );
        await pumpEventQueue();
    ...
    test(
      'dashboardSnapshotProvider automatically re-synthesizes state when streams emit changes',
      () async {
        ...
        expect(
          lastSnapshot.academicSnapshot.deadlinesState,
          contains('Due Today'),
        );
      },
    );
  ...
  group('Group 2: SafeBunkCalculator Mathematical Boundary & Stress Tests', () {
    ...
    test('Boundary 4: < 75% attendance needing mustAttend calculations', () {
      ...
      final verification =
          (50 + res50.mustAttend) / (100 + res50.mustAttend) * 100.0;
  ...
  group('Group 4: Rapid Concurrency Stress Tests', () {
    ...
    test(
      'Performs 100 concurrent db operations across tables without deadlock or failure',
      () async {
        final assignmentsRepo = container.read(assignmentRepositoryProvider);
        final calendarRepo = container.read(calendarRepositoryProvider);
