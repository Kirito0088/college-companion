...
void main() {
  ...
  group('Empirical Verification 1: Stream Emission Responsiveness', () {
    test(
      'Rapid concurrent inserts and updates trigger accurate stream notifications without loss or race conditions',
      () async {
        const userId = 'user_stress_1';
        final receivedEvents = <List<AssignmentEntity>>[];
        final subscription = assignmentRepository
            .watchAll(userId)
        ...
        final completedCount = latestList
    ...
    test(
      'Multiple concurrent table stream subscriptions remain responsive under heavy write load',
      () async {
        ...
        final sub3 = attendanceRepository
            .watchAll(userId)
            .listen(attEvents.add);
  ...
  group(
    'Empirical Verification 2: Soft-Delete Filtering across Domain Repositories',
    () {
      const userId = 'user_soft_delete_test';
      final now = DateTime.now().toUtc().toIso8601String();

      test(
        'UserRepository and UserSettingsRepository manage profile and settings tables',
        () async {
      ...
      test('Semesters repository excludes deletedAt != null records', () async {
        ...
        final getDeleted = await semesterRepository.getById(
          userId,
          'sem_deleted',
        );
      ...
      test('Subjects repository excludes deletedAt != null records', () async {
        ...
        final getDeleted = await subjectRepository.getById(
          userId,
          'sub_deleted',
        );
        expect(getDeleted, isNull);
      ...
      test(
        'Assignments repository excludes deletedAt != null records',
        () async {
          ...
          final getDeleted = await assignmentRepository.getById(
            userId,
            'asgn_deleted',
          );
          expect(getDeleted, isNull);
      ...
      test(
        'Attendance repository excludes deletedAt != null records',
        () async {
          ...
          final getDeleted = await attendanceRepository.getById(
            userId,
            'att_deleted',
          );
          expect(getDeleted, isNull);
      ...
      test('CalendarRepository excludes deletedAt != null records', () async {
        ...
        final getDeleted = await calendarRepository.getById(
          userId,
          'cal_deleted',
        );
        expect(getDeleted, isNull);
      ...
      test(
        'InternalMarksRepository excludes deletedAt != null records',
        () async {
          ...
          final getDeleted = await internalMarksRepository.getById(
            userId,
            'mark_deleted',
          );
          expect(getDeleted, isNull);
      ...
      test('ResourcesRepository excludes deletedAt != null records', () async {
        ...
        final getDeleted = await resourcesRepository.getById(
          userId,
          'res_deleted',
        );
        expect(getDeleted, isNull);
      ...
      test('TimetableRepository excludes deletedAt != null records', () async {
        ...
        final getDeleted = await timetableRepository.getById(
          userId,
          'tt_deleted',
        );
        expect(getDeleted, isNull);
      ...
      test(
        'Cascade soft-delete from Semester soft-deletes subjects and all downstream domain records',
        () async {
          await semesterRepository.create(
            SemestersCompanion(
          ...
          final asgn = await assignmentRepository.getById(
            userId,
            'asgn_cascade',
          );
  ...
  group(
    'Empirical Verification 3: Transaction Boundary & Atomic Rollback Integrity',
    () {
      const userId = 'user_tx_test';
      final now = DateTime.now().toUtc().toIso8601String();

      test(
        'Database transaction rolls back completely when an error occurs mid-transaction',
        () async {
          ...
          final rolledBackSem = await semesterRepository.getById(
            userId,
            'sem_tx_2',
          );
          ...
          final initialSem = await semesterRepository.getById(
            userId,
            'sem_tx_1',
          );
      ...
      test(
        'SemesterRepository.setCurrent maintains single active current semester atomically',
        () async {
          await semesterRepository.create(
            SemestersCompanion(
          ...
          final sem1 = await semesterRepository.getById(
            userId,
            'sem_current_1',
          );
          final sem2 = await semesterRepository.getById(
            userId,
            'sem_current_2',
          );
      ...
      test(
        'SubjectRepository.delete rolls back all child tables if error occurs during cascade',
        () async {
          await subjectRepository.create(
            SubjectsCompanion(
          ...
          // Verify before
          final beforeSub = await subjectRepository.getById(
            userId,
            'sub_tx_fail',
          );
          ...
          final afterSub = await subjectRepository.getById(
            userId,
            'sub_tx_fail',
          );
          final afterAsgn = await assignmentRepository.getById(
            userId,
            'asgn_tx_fail',
          );
  ...
  group('Empirical Verification 4: Foreign Key Indexing & Query Performance', () {
    test(
      'SQLite Query Planner utilizes indices for `@TableIndex` columns across all schema tables',
      () async {
        // Run EXPLAIN QUERY PLAN on queries matching `@TableIndex` definitions

        // 1. idx_subjects_user_deleted
        final subjectPlan = await database
            .customSelect(
              "EXPLAIN QUERY PLAN SELECT * FROM subjects WHERE user_id = 'u1' AND deleted_at IS NULL",
            )
            .get();
        final subjectPlanStr = subjectPlan
            .map((r) => r.data['detail'].toString())
            .join('\n');
        expect(
          subjectPlanStr.toLowerCase(),
          contains('using index idx_subjects_user_deleted'),
        );

        // 2. idx_assignments_user_status
        final asgnPlan = await database
            .customSelect(
              "EXPLAIN QUERY PLAN SELECT * FROM assignments WHERE user_id = 'u1' AND status = 'pending' AND deleted_at IS NULL",
            )
            .get();
        final asgnPlanStr = asgnPlan
            .map((r) => r.data['detail'].toString())
            .join('\n');
        expect(
          asgnPlanStr.toLowerCase(),
          contains('using index idx_assignments_user_status'),
        );

        // 3. idx_assignments_subject
        final asgnSubPlan = await database
            .customSelect(
              "EXPLAIN QUERY PLAN SELECT * FROM assignments WHERE subject_id = 's1'",
            )
            .get();
        final asgnSubPlanStr = asgnSubPlan
            .map((r) => r.data['detail'].toString())
            .join('\n');
        expect(
          asgnSubPlanStr.toLowerCase(),
          contains('using index idx_assignments_subject'),
        );

        // 4. idx_attendance_user_date
        final attPlan = await database
            .customSelect(
              "EXPLAIN QUERY PLAN SELECT * FROM attendance WHERE user_id = 'u1' AND date = '2026-07-24' AND deleted_at IS NULL",
            )
            .get();
        final attPlanStr = attPlan
            .map((r) => r.data['detail'].toString())
            .join('\n');
        expect(
          attPlanStr.toLowerCase(),
          contains('using index idx_attendance_user_date'),
        );

        // 5. idx_semesters_user_deleted
        final semPlan = await database
            .customSelect(
              "EXPLAIN QUERY PLAN SELECT * FROM semesters WHERE user_id = 'u1' AND deleted_at IS NULL",
            )
            .get();
        final semPlanStr = semPlan
            .map((r) => r.data['detail'].toString())
            .join('\n');
        expect(
          semPlanStr.toLowerCase(),
          contains('using index idx_semesters_user_deleted'),
        );

        // 6. idx_timetable_user_day
        final ttPlan = await database
            .customSelect(
              "EXPLAIN QUERY PLAN SELECT * FROM timetable WHERE user_id = 'u1' AND day_of_week = 1 AND deleted_at IS NULL",
            )
            .get();
        final ttPlanStr = ttPlan
            .map((r) => r.data['detail'].toString())
            .join('\n');
        expect(
          ttPlanStr.toLowerCase(),
          contains('using index idx_timetable_user_day'),
        );

        // 7. idx_calendar_events_user_date
        final calPlan = await database
            .customSelect(
              "EXPLAIN QUERY PLAN SELECT * FROM calendar_events WHERE user_id = 'u1' AND start_date = '2026-08-01' AND deleted_at IS NULL",
            )
            .get();
        final calPlanStr = calPlan
            .map((r) => r.data['detail'].toString())
            .join('\n');
        expect(
          calPlanStr.toLowerCase(),
          contains('using index idx_calendar_events_user_date'),
        );

        // 8. idx_internal_marks_subject
        final marksPlan = await database
            .customSelect(
              "EXPLAIN QUERY PLAN SELECT * FROM internal_marks WHERE subject_id = 'sub1' AND deleted_at IS NULL",
            )
            .get();
        final marksPlanStr = marksPlan
            .map((r) => r.data['detail'].toString())
            .join('\n');
        expect(
          marksPlanStr.toLowerCase(),
          contains('using index idx_internal_marks_subject'),
        );

        // 9. idx_resources_subject
        final resPlan = await database
            .customSelect(
              "EXPLAIN QUERY PLAN SELECT * FROM resources WHERE subject_id = 'sub1' AND deleted_at IS NULL",
            )
            .get();
        final resPlanStr = resPlan
            .map((r) => r.data['detail'].toString())
            .join('\n');
        expect(
          resPlanStr.toLowerCase(),
          contains('using index idx_resources_subject'),
        );

        // 10. idx_user_settings_user
        final settingsPlan = await database
            .customSelect(
              "EXPLAIN QUERY PLAN SELECT * FROM user_settings WHERE user_id = 'u1'",
            )
            .get();
        final settingsPlanStr = settingsPlan
            .map((r) => r.data['detail'].toString())
            .join('\n');
        expect(
          settingsPlanStr.toLowerCase(),
          contains('using index idx_user_settings_user'),
        );
      },
    );

    test(
      'Query benchmark demonstrates fast sub-millisecond lookups on indexed tables with 1000 records',
      () async {
            ...
            await database
                ...
                .insert(
                  AttendanceCompanion(
                    ...
                    date: Value(
                      '2026-07-${(1 + (i % 28)).toString().padLeft(2, '0')}',
                    ),
                    primaryStatus: const Value('present'),
                    lectureType: const Value('theory'),
                    createdAt: Value(now),
        ...
        final results = await attendanceRepository
            .watchOnDate(targetUserId, DateTime(2026, 7, 15))
            .first;
        stopwatch.stop();
        ...
        expect(
          ...
          lessThan(100),
        ); // Sub-100ms on in-memory VM (typically <5ms)
      },
    );
  });
}
