...
void main() {
  ...
  test(
    'create and getById attendance record with sync queue integration',
    () async {
  ...
  test(
    'delete soft-deletes attendance record and enqueues sync DELETE',
    () async {
      final now = DateTime.now().toUtc().toIso8601String();
  ...
  test(
    'watchBySubject, watchOnDate, watchByDateRange, and watchById filter properly',
    () async {
      final now = DateTime.now().toUtc().toIso8601String();
      ...
      // watchOnDate
      final dateList = await repository
          .watchOnDate('user_1', DateTime(2026, 7, 22))
          .first;
      ...
      final single = await repository
          .watchById('user_1', 'att_day1_subA')
          .first;
